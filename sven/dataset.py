...
from sven.constant import SEC_LABEL, VUL_LABEL, PROMPTS, CWES_TRAINED, CWES_TRAINED_SUBSET, CWE_TO_ID
...
class PrefixDataset(DatasetBase):
    ...
    def add_data(self, label, src, changes, vul_id, lang):
        cwe_name = CWES_TRAINED[vul_id]
        cwe_id = CWE_TO_ID[cwe_name]
        if label == SEC_LABEL:
            control_id = 0
            paired_id = cwe_id
        else:
            control_id = cwe_id
            paired_id = 0
        data = self.get_tensor(src, control_id, paired_id, changes)
        if data is not None:
            self.dataset.append(data)


    def get_tensor(self, src, control_id, paired_id, changes):
        ...
        return tokens, weights, control_id, paired_id
...
class TextPromptDataset(DatasetBase):
    ...
    def add_data(self, label, src, changes, vul_id, lang):
        prompt_idx = 0 if label == SEC_LABEL else 1
        if lang == 'py':
            control = get_indent(src) + '# ' + PROMPTS[prompt_idx]
        else:
            control = get_indent(src) + '// ' + PROMPTS[prompt_idx]
        src = control + src
